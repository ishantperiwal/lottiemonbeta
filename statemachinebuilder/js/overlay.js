// --- Global/Accesssssible Scope Variables ---
let originalData = null;
 let originalFilename = 'animation';
 let rawOriginalFileName = 'animation';
const maxSegments = 30;
const n = 2; // Minimum allowed segment duration in frames
const segmentColors = [
  '#87d8c0', // Mint Green
  '#c5e0a5', // Pastel Olive
  '#ffb79a', // Soft Peach
...
let deletedSegmentNames = []; // NEW: To track deleted segments for synchronization
let mainPlaying = false;
let renamedSegmentsLog = [];
...
window.addEventListener('message', (e) => {
  ...
  } else if (e.data.type === 'editSegment') { // <<< NEW LOGIC
    const segmentNameToEdit = e.data.segmentName;
    console.log(`Received request to edit segment: ${segmentNameToEdit}`);
    
    // Use a timeout to ensure the DOM has been rendered by initAnim
    setTimeout(() => {
        // Find the input element with the matching name
        const allNameInputs = [...segmentsContainer.querySelectorAll('.segment-name')];
        const targetInput = allNameInputs.find(input => input.value === segmentNameToEdit);

        if (targetInput) {
            const segmentControl = targetInput.closest('.segment-control');
            if (segmentControl) {
                // Check if it's already the first element. If not, click its overlay.
                if (segmentsContainer.firstChild !== segmentControl) {
                    const overlay = segmentControl.querySelector('.segment-overlay');
                    if (overlay) {
                        overlay.click();
                    } else {
                        console.warn("Could not find overlay to click for reordering.");
                    }
                }
            }
        } else {
            console.warn(`Segment with name "${segmentNameToEdit}" not found in the UI.`);
        }
    }, 500); // Delay to let renderSegments() finish
  }
...
function initAnim(data) {
  ...
  deletedSegmentNames = []; // NEW: Reset on new animation load
  renamedSegmentsLog = []; 
  mainPlaying = false;
...
document.getElementById('addSegment').onclick = () => {
    ...
    if (totalFrames < n) {
        alert(`Animation is too short to create a segment (minimum ${n} frames required).`);
        return;
    }
    ...
    const id = Date.now();
    const minDuration = n;
    const defaultEnd = Math.min(totalFrames, Math.max(minDuration, Math.floor(totalFrames * 0.25)));
...
function renderSegments() {
    ...
    segments.forEach(seg => {
        // --- NEW: Do not render the control div for "Full Animation" ---
        if (seg.name === 'Full Animation') {
            updateHighlight(seg); // Still update its highlight on the global bar
            return; // But skip creating the DOM element
        }
...
function createSegmentElement(seg) {
 ...
 function attachSegmentHandlers() {
     ...
     segmentsContainer.querySelectorAll('.range-input').forEach(input => {
        ...
        input.onclick = handleSegmentRangeInput; // Also trigger on a simple click
   ...
   /* --------------- */
    ...
    segmentsContainer.querySelectorAll('.segment-control').forEach(segEl => {
        const handles = segEl.querySelector('.segment-handles');
        if (handles) {
            handles.addEventListener('mouseenter', handleSegmentHandlesMouseEnter);
        }
        segEl.addEventListener('mouseleave', handleSegmentControlMouseLeave);
    });
...
function attachSingleSegmentHandlers(segmentElement) {
    ...
    segmentElement.querySelector('.close-btn').onclick = handleRemoveSegment;
     ...
     const handles = segmentElement.querySelector('.segment-handles');
    if (handles) {
        handles.addEventListener('mouseenter', handleSegmentHandlesMouseEnter);
    }
    segmentElement.addEventListener('mouseleave', handleSegmentControlMouseLeave);
}
...
// --- Event Handlers for Segment Controls ---
 function handleSegmentNameInput(e) {
    ...
    if (isDuplicate) {
        ...
        // --- START: NEW LOGIC to track the rename ---
        // Check if this segment's *previous* name is already the 'newName' in our log.
        // This handles chain renames (e.g., A -> B, then B -> C).
        const existingRename = renamedSegmentsLog.find(r => r.newName === oldName);

        if (existingRename) {
            // If we're renaming "B" to "C", and we have a log for "A" -> "B",
            // we update it to be "A" -> "C". This preserves the original name.
            existingRename.newName = newName;
            console.log(`[SYNC] Updated rename log: '${existingRename.oldName}' is now '${existingRename.newName}'`);
        } else {
            // This is the first time this segment has been renamed in this session.
            // Log the change from its original name to the new name.
            renamedSegmentsLog.push({ oldName: oldName, newName: newName });
            console.log(`[SYNC] Logged new rename: '${oldName}' -> '${newName}'`);
        }

        // Finally, update the name in the main data model
        segToUpdate.name = newName;
        e.target.value = newName; // Also update the input in case it was trimmed
        // --- END: NEW LOGIC ---
    }
}
 function handleSegmentFrameInput(e) {
     ...
     const minDuration = n; // Minimum segment duration in frames

     if (type === 'start') {
         val = Math.max(0, Math.min(val, totalFrames - minDuration)); // Clamp start frame
         seg.start = Math.min(val, seg.end - minDuration); // Ensure start is `minDuration` from end
     } else { // type === 'end'
         val = Math.max(minDuration, Math.min(val, totalFrames)); // Clamp end frame
         seg.end = Math.max(val, seg.start + minDuration); // Ensure end is `minDuration` from start
     }
     // Ensure values don't cross after clamping and maintain min duration
     seg.start = Math.min(seg.start, seg.end - minDuration);
     seg.end = Math.max(seg.end, seg.start + minDuration);
      fullSeek.value = val;
 ...
 function handleSegmentRangeInput(e) {
    // --- PAUSING LOGIC ---
    // 1. Pause main animation if it's playing
    if (mainPlaying) {
        animation.pause();
        mainPlaying = false;
    ...
    // 2. Pause segment animation if it's playing
    if (selectedSegmentId !== null) {
        animation.pause();
        const previousButton = document.querySelector(`.play-btn.paused[data-id='${selectedSegmentId}']`);
        if (previousButton) {
        ...
        // Restore default listeners for manual seeking, which is the expected next action
        animation.removeEventListener('enterFrame', updateSeekOnEnterFrame_Segment);
    ...
    if (type === 'start') {
        seg.start = Math.min(val, seg.end - minDuration); // Prevent handle from getting too close to end
        finalVal = seg.start;
    } else { // type === 'end'
        seg.end = Math.max(val, seg.start + minDuration); // Prevent handle from getting too close to start
        finalVal = seg.end;
    }

    if (animation && finalVal >= 0 && finalVal <= totalFrames) {
        // 1. Update the main seek bar's value
        fullSeek.value = finalVal;

        // 2. Go to the corresponding frame in the animation preview
        animation.goToAndStop(finalVal, true);

        // 3. Update and show the tooltip for the main seek bar
        updateTooltip(finalVal);
...
function handleSegmentRangeChange(e) {
    ...
    seg.start = Math.max(0, Math.min(seg.start, totalFrames - minDuration));
    seg.end = Math.max(seg.start + minDuration, Math.min(seg.end, totalFrames));
    seg.start = Math.min(seg.start, seg.end - minDuration); // Final cross-check
...
function drawGlobalBar() {
  ...
  segments.forEach(seg => {
    ...
    // --- NEW: Manually set color for "Full Animation" segment ---
    if (seg.name === 'Full Animation') {
        d.style.background = '#00000027'; // A dark, almost black color
    }
...
if (segmentIndex > -1) {
        // --- NEW: Store the name of the segment before it's removed from the array ---
        const deletedSegmentName = segments[segmentIndex].name;
        // We don't need to track the special "Full Animation" segment if it were deletable
        if (deletedSegmentName && deletedSegmentName !== 'Full Animation') {
            deletedSegmentNames.push(deletedSegmentName);
            console.log(`[SYNC] Marked segment '${deletedSegmentName}' for deletion sync.`);
        }
 ...
 function handleApplyAndClose() {
    ...
    // Post the message to the parent window
     window.parent.postMessage({
        ...
        deletedSegmentNames: deletedSegmentNames, // Send the list of deleted names
        renamedSegments: renamedSegmentsLog // <-- ADD THIS LINE
...
window.addEventListener('message', (event) => {
    ...
    function handleSegmentHandlesMouseEnter(e) {
    const segmentControl = e.currentTarget.closest('.segment-control');
    const overlay = segmentControl.querySelector('.segment-overlay');
    if (overlay) {
...
function handleSegmentControlMouseLeave(e) {
    const overlay = e.currentTarget.querySelector('.segment-overlay');
    if (overlay) {
...
// --- NEW --- Functions to shandle segment overlays and reordering
function updateOverlays() {
    const segmentElements = [...segmentsContainer.children]; // Get a static array of children
    segmentElements.forEach((segEl, index) => {
        let overlay = segEl.querySelector('.segment-overlay');

        if (index === 0) {
            // This is the first/active segment. If it has an overlay, remove it.
            if (overlay) {
                overlay.remove();
            }
        } else {
            // This is an inactive segment. It MUST have an overlay.
            if (!overlay) {
            ...
            // Ensure the 'locked' class is not present on standard overlays.
